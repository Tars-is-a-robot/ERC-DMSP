...
def train_or_eval_model(model, loss_f, dataloader, train_flag=False, optimizer=None, cuda_flag=False, feature_type='text',
    ...
    losses, preds, labels, Group_character, length, people= [], [], [], [], [], []
    if train_flag:
        model.train()
    else:
        model.eval()
    ...
    for step, data in enumerate(dataloader):
        ...
        log_prob, _, _, print_in = model(roberta_fea, qmask, seq_lengths)
        Group_character.append(print_in)
        length.append(seq_lengths)
        people.append(qmask)
    ...
    return avg_loss, avg_accuracy, avg_fscore, all_matrix, [labels, preds], Group_character, length, people
...
if __name__ == '__main__':
    ...
    parser.add_argument('--loss', default="NLLLoss", help='loss function: FocalLoss/NLLLoss')

    parser.add_argument('--class_weight', action='store_true', default=True, help='use class weights')

    parser.add_argument('--tensorboard', action='store_true', default=False, help='enables tensorboard log')

    parser.add_argument('--seed', type=int, default=107, help='random seed')

    parser.add_argument('--use_cognition', action='store_true', default=True, help='是否使用认知网络')

    parser.add_argument('--input_size', type=int, default=1024, help='the number of reason turns at situation-level')

    parser.add_argument('--hidden_size', type=int, default=128, help='the number of reason turns at situation-level')

    parser.add_argument('--n_speakers', type=int, default=2, help='the number of reason turns at situation-level')

    parser.add_argument('--n_classes', type=int, default=6, help='the number of reason turns at situation-level')

    parser.add_argument('--marge_speaker', action='store_true', default=False, help='是否使用说话人性格融合')

    parser.add_argument('--g_n_layers', type=int, default=1, help='全局话语性格参与层数')

    parser.add_argument('--p_n_layers', type=int, default=2, help='个人话语性格参与层数')
    ...
    # class_weights = torch.FloatTensor([1/0.086747, 1/0.144406, 1/0.227883, 1/0.160585, 1/0.127711, 1/0.252668])
    ...
    model = DialogueCRN(cuda_flag=cuda_flag,
                        args=args)
    if cuda_flag:
    ...
    if status == 'train':
        ...
        save_path = "tensor_G_c.pth"
        lengh_path = "lengh_G_c.pth"
        people_path = "people_G_c.pth"


        for e in range(epochs):
            start_time = time.time()

            train_loss, train_acc, train_fscore, _, _, train_G_c, train_lengh, train_people = train_or_eval_model(model=model, loss_f=loss_f, dataloader=train_loader, train_flag=True,
                                                                            optimizer=optimizer, cuda_flag=cuda_flag, feature_type=feature_type,
                                                                            target_names=target_names)
            valid_loss, valid_acc, valid_fscore, _, _, eval_G_c, eval_lengh, eval_people = train_or_eval_model(model=model, loss_f=loss_f, dataloader=valid_loader, cuda_flag=cuda_flag,
                                                                            feature_type=feature_type, target_names=target_names)
            test_loss, test_acc, test_fscore, test_metrics, _, test_G_c, test_lengh, test_people= train_or_eval_model(model=model, loss_f=loss_f, dataloader=test_loader,
            ...
            if args.valid_rate > 0:
                eval_loss, _, eval_fscore = valid_loss, valid_acc, test_fscore
            else:
            ...
            if e == 0 or best_eval_fscore < eval_fscore:
                ...
                torch.save(train_G_c, save_path)
                torch.save(train_lengh, lengh_path)
                torch.save(train_people, people_path)
            else:
                patience += 1
