...
def train_or_eval_model(model, loss_f, dataloader, train_flag=False, optimizer=None, cuda_flag=False, feature_type='text', target_names=None):
    ...
    for data in dataloader:
        ...
        log_prob, lambda_a, lambda_b = model(roberta_fea, qmask, seq_lengths)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch-size', type=int, default=32, metavar='BS', help='batch size') # 32
    ...
    parser.add_argument('--n_speakers', type=int, default=9, help='the number of reason turns at situation-level')

    parser.add_argument('--n_classes', type=int, default=7, help='the number of reason turns at situation-level')

    parser.add_argument('--marge_speaker', action='store_true', default=False, help='是否使用说话人性格融合')

    parser.add_argument('--g_n_layers', type=int, default=2, help='全局话语性格参与层数')

    parser.add_argument('--p_n_layers', type=int, default=3, help='个人话语性格参与层数')
    ...
    if args.cls_type.strip().lower() == 'emotion':
        ...
        class_weights = torch.FloatTensor([1 / 0.469506857, 1 / 0.119346367, 1 / 0.026116137, 1 / 0.073096002, 1 / 0.168368836, 1 / 0.026334987, 1 / 0.117230814])
        # class_weights = torch.FloatTensor([0.30427062, 1.19699616, 5.47007183, 1.95437696,
